...
We will compare two of the bigger programming paradigms, object-oriented programming and functional programming. To compare these we have chosen four different algorithms which we each will implement twice. Once according to OOP and once according to FP. The different algorithms we will implement is tree search algorithms, shell sort, tower of hanoi and Dijkstra's algorithm. The comparison we will make between the algorithms is runtime, memory usage, time of development and lines of code. All algorithms will be written in JavaScript.
...
With the rise of popularity of functional programming and the dominance of object-oriented programming, of course people is starting to ask the question,”Which paradigm is the best?”. Some studies have tried to answer this question, by comparing different programming languages, but while comparing this you also compare different languages in general, different compilers and so forth. This thesis will take the question one step further, by writing both paradigms in JavaScript, the comparison can be made without having to compare a lot of different 
...
There is a call stack limit in JavaScript that varies depending on the runtime environment. This limits the number of function calls that can be made, which also limits recursion. In ES6 there is tail call optimization that makes it possible to make certain function calls without adding to the call stack, that would allow for better recursing. However this is not implemented for most servers and browsers yet [17].
...
We will implement the algorithms using Atom, a text editor, and compile and run our tests and our code with Node.js. Tail call optimization is not available in Node.js, see \ref{sec:js-fp} manage our dependencies we are using npm. For automation we are using Grunt.
