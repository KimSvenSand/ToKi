...
Kim's functional implementation is faster than her object-oriented since her inOrderTraversal-function is unnecessarily complex and does not fully utilize the idea of the recursive algorithm seen in section~\ref{sec:binary-tree}. Her recursive algorithm uses more memory because we are handling our data structures as immutable. This means that we are copying our data structures recursively when initiating an array or a tree, which is handled as all of our copies being placed into memory. We suspect that this would have looked different other languages that support immutable data structures. 

The reason that Tord’s functional algorithm is so much slower is that he in this solution uses arrays instead of native objects. The problem with arrays is not the array itself but how it is copied with the slice()\cite(jsref) function, to represent immutable data. This function have we found out afterwards, is not very effective.
