...
\begin{itemize}
\item Tests for the algorithm that are defined in AppendixA placed in <projectName>/es6-test
\item A complete implementation of the algorithm that has passed the tests placed in <projectName>/src/js
\item A measurement implementation that must run the algorithm and measure time and memory usage using node.js process. For the object-oriented implementation a UML diagram shall also be provided.
